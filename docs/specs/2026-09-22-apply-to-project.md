...
- 日期：2026-09-22
- Issue：#14
- 状态：待实施（spec PR #13 已合并；spec 期审核档位 owner 点「不审」）

## 背景：要解什么问题

多项目遵循 spec-flow 的现行做法是「逐字副本 + `@` 导入」，同步已成本烂账：

1. **副本会静默过期**——neuron-entry 的 `docs/spec-flow.md` 是 SKILL.md 的逐字副本，停在真身提交 `2c329cd`（2026-09-21），当天即落后；xhs-analysis 同形制。真身每改一次，N 个副本仓都要记得重拷（2026-09-22 逐仓 grep 实测）。
...
ADR 疑虑的拍板（2026-09-22 对话）：owner 担心「外部没有指示让项目知道自己开了 ADR」。解法：ADR 入口节落在 AGENTS.md（两家工具每会话自动注入，非 agent 自觉读取），且入口节写成**动作式指令**（开工前搜 / 拍板后写），不是干声明。实证：finance-workspace 2026-09-22 启用 ADR 当天落 10 条决策记录，靠的就是 AGENTS.md 里的动作指令。
...
加一节「2026-09-22 演进」：「严格遵循的项目」层的标准动作从「同步一份 + 指向真身」演进为「**应用到项目**功能产出的只指向式」；存量副本仓（neuron-entry / xhs-analysis）按 owner 2026-09-22 拍板**维持现状、不追改**；三层模型其余部分不改写。
...
- ADR 重型机制（三时点 Gate、消费者反查、检查脚本、模板生成器）不预装（ADR 备忘录§八）。
...
- [ ] `references/adoption-model.md` 含「2026-09-22 演进」节，三层模型其余部分未改写。
...
- [ ] 回响检查（第 5 步硬规矩）：SKILL.md 内引用「参考」「adoption-model」的落点逐一核对，有漂移才同步。
...
| D2 | worktree 根 `~/code/spec-flow.worktrees/`、分支 `docs/apply-to-project` | 同仓既有惯例（无 issue 号的活用 `<type>/<slug>`） |
