...
- 日期：2026-09-22
- Issue：#14
- 状态：已实施（实现见 PR #17；spec 期后补轻审 8 条 + 实施期轻审 13 条，全数修订，见文末「审核修订记录」）
...
7. **旧副本检测口径**：命中真身的**特征句**（分散全文的一组标志句，实现列 7 条）判为逐字 / 整段摘抄副本；或指令文件已有**复述六步流程正文**的段落（只提 spec-flow 名字的指针句不算）。**不用相似度阈值**（owner 2026-09-22 追认，理由见 D8）。处置一律为「报告 + 用户定夺」。
...
| D8 | 「旧副本」判定用**特征句命中**（真身全文分散的一组标志句，实现列 7 条），不用相似度阈值、不做人肉逐字比对 | 可操作的判定口径；误报代价低（只报告）。**原写「高相似（diff / grep 判）」，2026-09-22 实施期实测后改**：真·过期副本与当前真身行数差 28%，按阈值会漏掉恰好要抓的那个案例；**owner 2026-09-22 追认** |
...
### 实施期轻审（2026-09-22，PR #17）

1 个独立 agent（实施期轻审 = 对抗式找 bug + 范围外对账），13 条发现全数修订：

| # | 严重度 | 发现 | 处置 |
|---|---|---|---|
| 1 | 严重 | §五 规则页骨架把 ADR 备忘录的规则正文整段复述（且残缺），与 D7「只链不复述」直接冲突 | 骨架砍到 D7 四条：启用声明 + 载体分工 + 命名默认 + 其余全链 |
| 2 | 中 | §一 幂等校验用字面匹配，被包进反引号的 `@AGENTS.md` 被判「已有」→ 静默跳过补写 | 判定改「生效形态」（`^@AGENTS.md$` 锚定，排除反引号 / 代码块） |
| 3 | 中 | §一 标志句口径自相矛盾：单看「唯一真相源」会把无关仓判成「已应用」→ 指针永不写 | 改**合取**：须同时命中 spec-flow 名字 + 声明句 |
| 4 | 中 | §七 旧流程段口径在指针块上误报（「六步流程」是指针块自己的句子） | 改「复述流程**正文**」，显式排除只提名字的指针句 |
| 5 | 中 | 判定树不问符号链接：软链自指 / 指向仓外文件两种实测写坏 | §一 加前置检测，两种情形各给动作 |
| 6 | 中 | 追加接缝没规定：目标文件末行无换行时模板首行标题被吞 | §一 加接缝要求（补换行 + 追加前留空行） |
| 7 | 中 | 缺「项目自带 ADR 规则页在别处」态 → 会造出第二份规则页 | §四 加前置判断；入口节规则页路径改占位符 |
| 8 | 轻 | 指针块把采纳模型说成「三种」且自行改名，会写进 N 个仓 | 对齐 `adoption-model.md` 的四项规范措辞 |
| 9 | 轻 | §七 特征句抓不到「只抄中间一段」的摘抄 | 特征句扩到 7 条、分散全文；标签改「整段摘抄」并写明边界 |
| 10 | 轻 | SKILL.md 与细则对「旧副本」的宽窄不一 | SKILL.md 那句同步为「逐字 / 整段摘抄副本」 |
| 11 | 轻 | dogfood 证据只覆盖一半模板（出货版新措辞没测过） | 用出货版模板重跑五形态 |
| 12 | 轻 | PR 报备「回响同步 2 处」实际 3 处 | 账目补全 |
| 13 | 轻 | D8 判据被换成特征句、只报「已修」未摆拍板 | owner 2026-09-22 追认；改动面项 7 与 D8 同步改写 |
